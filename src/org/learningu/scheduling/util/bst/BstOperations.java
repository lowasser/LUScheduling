...
final class BstOperations {
  ...
  /**
   * Returns the result of performing the mutation specified by {@code mutationRule} in
   * {@code tree} at the location with key {@code key}.
   * 
   * <ul>
   * <li>If the returned {@link BstModificationResult} has type {@code IDENTITY}, the exact
   * original tree is returned.
   * <li>If the returned {@code BstModificationResult} has type {@code REBUILDING_CHANGE}, the tree
   * will be rebuilt with the node factory of the mutation rule, but not rebalanced.
   * <li>If the returned {@code BstModificationResult} has type {@code REBALANCING_CHANGE}, the
   * tree will be rebalanced using the balance policy of the mutation rule.
   * </ul>
   */
  public static <K, N extends BstNode<K, N>> BstMutationResult<K, N> mutate(
      Comparator<? super K> comparator,
      BstMutationRule<K, N> mutationRule,
      @Nullable N tree,
      @Nullable K key) {
    checkNotNull(comparator);
    ...
    if (tree != null) {
      ...
      if (cmp != 0) {
        BstSide side = (cmp < 0) ? LEFT : RIGHT;
        BstMutationResult<K, N> mutation = mutate(
            comparator,
            mutationRule,
            tree.childOrNull(side),
            key);
        return mutation.lift(
            tree,
            side,
            mutationRule.getNodeFactory(),
            mutationRule.getBalancePolicy());
      }
    }
    ...
    switch (modResult.getType()) {
      ...
      case REBUILDING_CHANGE:
        if (modResult.getChangedTarget() != null) {
          changedRoot = nodeFactory.createNode(
              modResult.getChangedTarget(),
              originalLeft,
              originalRight);
        } else if (tree == null) {
          changedRoot = null;
  ...
  public static <K, N extends BstNode<K, N>> BstMutationResult<K, N> extractMin(
    ...
    if (root.hasChild(LEFT)) {
      BstMutationResult<K, N> subResult = extractMin(
          root.getChild(LEFT),
          nodeFactory,
          balancePolicy);
  ...
  public static <K, N extends BstNode<K, N>> BstMutationResult<K, N> extractMax(
    ...
    if (root.hasChild(RIGHT)) {
      BstMutationResult<K, N> subResult = extractMax(
          root.getChild(RIGHT),
          nodeFactory,
          balancePolicy);
    ...
    return BstMutationResult.mutationResult(
        root.getKey(),
        root,
        root.childOrNull(LEFT),
        BstModificationResult.rebalancingChange(root, null));
  }

  /**
   * Inserts the specified entry into the tree as the minimum entry. Assumes that
   * {@code entry.getKey()} is less than the key of all nodes in the subtree {@code root}.
   */
  public static <N extends BstNode<?, N>> N insertMin(
      @Nullable N root,
      N entry,
      BstNodeFactory<N> nodeFactory,
      BstBalancePolicy<N> balancePolicy) {
  ...
  /**
   * Inserts the specified entry into the tree as the maximum entry. Assumes that
   * {@code entry.getKey()} is greater than the key of all nodes in the subtree {@code root}.
   */
  public static <N extends BstNode<?, N>> N insertMax(
      @Nullable N root,
      N entry,
      BstNodeFactory<N> nodeFactory,
      BstBalancePolicy<N> balancePolicy) {
