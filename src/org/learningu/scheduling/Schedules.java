...
public final class Schedules {
  ...
  private static void checkNoTeacherConflicts(Schedule schedule) {
    ...
    for (Map.Entry<TimeBlock, Map<Room, Course>> scheduleForBlock : schedule.getScheduleTable()
        .rowMap()
        .entrySet()) {
      TimeBlock block = scheduleForBlock.getKey();
      Map<Room, Course> roomAssignmentsForBlock = scheduleForBlock.getValue();
      Multiset<Teacher> teacherSchedules = HashMultiset.create(roomAssignmentsForBlock.size());
      for (Course scheduledCourse : roomAssignmentsForBlock.values()) {
        teacherSchedules.addAll(program.teachersForCourse(scheduledCourse));
      }
      ...
      for (Multiset.Entry<Teacher> entry : teacherSchedules.entrySet()) {
        ...
        checkArgument(entry.getCount() <= 1,
            "Teacher %s is scheduled to teach more than once in block %s",
            teacher,
            block);
        checkArgument(logic.isCompatible(teacher, block),
  ...
  private static void checkNoRoomConflicts(Schedule schedule) {
    ...
    for (Table.Cell<TimeBlock, Room, Course> assignment : schedule.getScheduleTable().cellSet()) {
      ...
      checkArgument(logic.isCompatible(course, room),
          "Course %s and room %s are incompatible",
          course,
          room);
    }
  }
  ...
  private static void checkCoursesScheduledOnce(Schedule schedule) {
    Multiset<Course> uniqueScheduledCourses = ImmutableMultiset.copyOf(schedule.getScheduleTable()
        .values());
    checkArgument(uniqueScheduledCourses.size() == uniqueScheduledCourses.elementSet().size(),
        "Courses are scheduled more than once in schedule %s",
        schedule);
  }
  ...
  private static void checkScheduleMatchesWithProgram(Schedule schedule) {
    for (TimeBlock block : schedule.getScheduleTable().rowKeySet()) {
      checkArgument(block.getProgram() == schedule.getProgram(),
          "Block is not associated with this program");
    }
    for (Room room : schedule.getScheduleTable().columnKeySet()) {
      checkArgument(room.getProgram() == schedule.getProgram(),
          "Room is not associated with this program");
    }
    for (Course course : schedule.getScheduleTable().values()) {
      checkArgument(course.getProgram() == schedule.getProgram(),
